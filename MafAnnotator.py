...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(argv, "hm:c:n:d:t:u:")
    ...
    for opt, arg in opts:
        if opt == '-h':
            print 'test.py -m <input MAF file> -n <output MAF file> [-c <input clinical file>] [-d <output clinical file>] [-t <default tumor type>] [-u base-url]'
    ...
    if outputmaffile == '' or outputmaffile=='':
        print 'for help: python MafAnnotator.py -h'
        sys.exit(2)
    ...
    if inputclinicalfile != '' and outputclinicalfile != '':
        processclinicaldata([outputmaffile], inputclinicalfile, outputclinicalfile)

    print 'done!'

if __name__ == "__main__":
    # argv = [
    #     '-m', 'data/example_maf.txt',
    #     '-n', 'data/example_maf.oncokb.txt',
    #     '-c', 'data/example_clinical.txt',
    #     '-d', 'data/example_clinical.oncokb.txt'
    # ]
    # main(argv)
