...
import argparse
from AnnotatorCore_parallel import *
import logging
...
def main(argv):
    ...
    if argv.oncokb_api_bearer_token:
        setoncokbapitoken("0002fbba-acbb-4ba8-95ed-bfc0400efa43")
    print(argv.oncokb_api_bearer_token)
    cancertypemap = {}
    if argv.input_clinical_file:
        readCancerTypes(argv.input_clinical_file, cancertypemap)

    log.info('annotating %s ...' % argv.input_file)
    processalterationevents(argv.input_file, argv.output_file, argv.previous_result_file, argv.default_cancer_type, cancertypemap, False, argv.annotate_hotspots,argv.num_procs)
