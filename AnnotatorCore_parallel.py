...
def processalterationevents(eventfile, outfile, previousoutfile, defaultCancerType, cancerTypeMap,
                            retainonlycuratedgenes, annotatehotspots, num_procs):
    
    dirname = os.path.dirname(__file__)
    table = os.path.join(dirname,'gene-entrez-table.csv')
    gene_symbol_entrez = open(table).readlines()
    symbol_entrez_dict = {i.split(",")[1].strip().replace('"',''):i.split(",")[0].strip().replace('"','') for i in gene_symbol_entrez}
    ...
    with open(eventfile, 'rU') as infile:
        ...
        for row in reader:
            ...
            if retainonlycuratedgenes and hugo not in curatedgenes:
                continue
            if hugo not in symbol_entrez_dict:
                flag=True
            if hugo in symbol_entrez_dict:
                hugo = symbol_entrez_dict[hugo]
            ...
            arg_list.append((hugo, hgvs, consequence, start, end, cancertype,i,flag))
            row_list.append(row)
        ...
        #output_string=output_string+'\t'.join(row) + "\n"
        print(len(arg_list))
        with Pool(num_procs) as p:
            results = p.starmap(pull_mutation_info, arg_list)
        #infile.seek(0)
        print(len(results))
        print(len(row_list))
        # Weird edge case handling in R file
        for row,oncokbinfo in zip(row_list,results):
            row.append(oncokbinfo)
            outf.write('\t'.join(row) + "\n")
        #print(results[0:50])
...
def pull_mutation_info(hugo, protein_change, consequence, start, end, cancer_type,i,symbol):
    if i%100==0:
        log.info(i)
    ...
    if key not in oncokbcache:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                #if evidences['query']['hugoSymbol'] != key.split('-')[0]:
                #    log.error('Incorrect hugo symbol queried for %s. Found %s instead' %(key.split('-')[0],evidences['query']['hugoSymbol']))
                #print(evidences)
